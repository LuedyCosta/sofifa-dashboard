...
def renderizar_perfil(df, find_similar_players, STAT_GROUPS, get_val):
    ...
    # -------------------------------------------------------------------------
    # SEÇÃO DE INDICADORES DE PERFORMANCE E COMPARAÇÃO NO RADAR
    # -------------------------------------------------------------------------
    st.markdown("### 2 · Indicadores de Performance")

    # Mapeamento plano de todos os atributos possíveis para seleção
    all_attributes = {}
    for group_name, attrs in STAT_GROUPS.items():
        for label_pt, col_en in attrs.items():
            all_attributes[f"{group_name} - {label_pt}"] = col_en

    # Chaves de controle no session_state para Marcar/Desmarcar Todos
    if "selected_indicators" not in st.session_state:
        # Padrão inicial com alguns atributos principais
    ...
    # Expander interativo para seleção de atributos por grupo
    ...
    # Opções de Comparação de Jogadores Adicionais no Radar
    ...
    # Construção do Gráfico de Radar Interativo
    ...
    # Exibição detalhada por abas tradicionais de atributos
