...
def renderizar_playstyles():
    ...
    for idx, cat in enumerate(categorias):
        with cols[idx % 3]:
            # Substituído st.container(border=True) por um bloco HTML usando a classe padrão .custom-box
            st.markdown(f"""
            ...
            with st.expander("Expandir"):
                ...
                for i, (nome_style, desc_style) in enumerate(items):
                    st.markdown(f"<span class='var-text'>{nome_style}</span>", unsafe_allow_html=True)
